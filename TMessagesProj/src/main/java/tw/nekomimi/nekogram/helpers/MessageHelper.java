...
public class MessageHelper extends BaseController {
    ...
    public MessageObject getOwnReply(MessageObject messageObject) {
        ...
        if (loaded != null && (loaded.scheduled
        ...
        // The header is the address even when its preview hasn't loaded. A detached reference also
        // avoids sendMessage redirecting a loaded forwarded target to that target's channel origin.
        TLRPC.TL_message reference = new TLRPC.TL_message();
        reference.id = header.reply_to_msg_id;
        reference.dialog_id = replyDialogId;
        reference.peer_id = getMessagesController().getPeer(replyDialogId);
        ...
        if (messageObject.hasValidReplyMessageObject()
                && loaded.currentAccount == currentAccount && loaded.getId() == reference.id
