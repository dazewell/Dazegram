...
public class HotkeyController {
    ...
    public static boolean handleTextStyleShortcut(EditTextCaption editText, int keyCode, KeyEvent event) {
        ...
        if (itemId == 0 || (needsSelection && selStart == selEnd)) {
            return false;
        }
        // A hardware-keyboard selection made bottom-to-top (Shift+Up) is "reversed":
        // getSelectionStart() (the anchor) is greater than getSelectionEnd() (the caret).
        // The makeSelected* methods feed these raw into MediaDataController.addStyleToText,
        // which only applies the span when start < end — so reversed selections silently
        // did nothing. Push a normalized range through the selection override (consumed once
        // by performMenuAction) so the formatting works regardless of selection direction.
        if (selStart >= 0 && selEnd >= 0) {
            editText.setSelectionOverride(Math.min(selStart, selEnd), Math.max(selStart, selEnd));
        }
