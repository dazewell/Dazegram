...
The five Nagram-branded entries in the app-icon picker — Blue, Default, Niello, Dark Blue and Blur Blue (Blue is the one selected out of the box, not the entry named "Default") — now draw a shared monoline "D" glyph (a stem and a bowl) instead of the old Nagram "N". The Official build's icon impersonates Telegram Beta's package and draws just the D; the Unofficial build (NekoX's package) adds a single diagonal chord through the same shape as its "X" distinguisher, so the two builds stay visually distinct at a glance. Default's plate switches from white to amber and its glyph to flat gunmetal, and Dark Blue's plate deepens from the shared bright blue to a darker navy, so neither becomes a near-duplicate of Niello or Blur Blue now that all five share one glyph shape. Every option's themed/monochrome icon (Android 13+) now draws from one shared silhouette asset instead of five inconsistent ones, fixing visible ghosting and shape defects some options previously had in that mode. No labels, filenames, package IDs or other picker entries changed — existing selections repaint in place on next launch, no re-selection needed.
