...
About five seconds before each cut you'll feel a short double-buzz and the recording ring starts pulsing, so you know a handover is coming even with the phone away from your face; a single sharper buzz marks the moment it actually happens. Nothing fires on the last minute or once infinite mode has dropped out from under you (view-once, slow mode), since those just stop normally. Turn vibration off in NekoGram settings and you still get the ring pulse. Also fixed: a word landing right on the cut used to get dropped from both the message that just closed and the one starting up, since the handover briefly treated the still-running recording as if it had just started over. That's gone now — the audio keeps flowing straight across the cut.
