...
Two more theme-strip tiles, Solid Light and Solid Dark, land right after Extera Dark (same Android 12+ dynamic row, same behaviour — colours re-derive on wallpaper change, neither is preselected, picked purely by tapping its tile, no new setting anywhere). They're otherwise identical to Extera Light/Dark; the only difference is the chat composer panel and the dialog/search input box use a flatter, fully opaque surface instead of Extera's wallpaper-tinted one, so those two surfaces stand out from the wallpaper hue rather than blending into it. Solid Light uses a near-white `#F5F5F5` there and Solid Dark a flat `#242424` — both deliberately chosen over pure white/black to keep the surfaces from looking stark against the rest of the Extera-derived palette. This is most noticeable with glass/blur effects off: with blur on, the panel is drawn at reduced alpha over the blurred wallpaper same as every other theme, so roughly 15–25% of the wallpaper hue still shows through by construction — that's the existing blur-tint behaviour, not this feature working incorrectly. Everything else — tile preview colours, every other surface, all ~720 remaining palette keys — is byte-for-byte the same as Extera Light/Dark. Both pairs ship and coexist; this isn't a replacement for Extera.
