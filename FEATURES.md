...
Setting a Panic Code that's the same as a code you type to get in would be a trap — a real duress entry would just unlock instead of logging out. So setup now keeps the Panic Code distinct from every normal unlock code: while you're setting or changing it, entering a code that matches your app passcode or any account's passcode is rejected on the spot, with a caption naming what it clashed with, and going the other way — changing your app or an account passcode to your current Panic Code — is refused too. App and account passcodes may still match each other; only the Panic Code has to be unique. Inactive accounts that still have a passcode saved count, so a logged-out slot can't hide a clash.
