...
The same sheet has a `Notifications` section with a `Disguise notifications` switch, off by default. While it is on, that chat uses a generic cover persona instead of real chat/sender/message content. You can pick the persona (`Cover`) and post a safe `Preview notification` from the sheet without changing unread state. The separate per-chat `Alert normally` row is gone: covered notifications now follow Telegram's own existing silent-vs-alert signal per event. A normal (non-silent) event alerts through the cover identity, while an upstream-silent event stays silent. Notification tuning still lives in the usual Telegram/Android controls (mute, per-chat sound, channel settings, watch settings), and a grouped busy covered batch can alert once per eligible covered chat because the cover summary intentionally stays in the silent tier.

The Chat Privacy sheet now uses the same rounded section-card surface: one card for `Hide last message` / `Placeholder text` / `Require password`, then one for `Disguise notifications` / `Cover` / `Preview notification`, with bare section headers and disclosure rows outside cards. To keep the sheet short by default without hiding active controls, the old always-open limitations footer is now a collapsed `How covers work` disclosure at the bottom. Expanding it shows the same limitations text in place. On short/landscape screens the sheet content scrolls, so every control and the full expanded text stay reachable, and row-triggered bulletins are positioned above the row that raised them even when scrolled.

Covered taps/dismissals only clear that represented cover and never open the chat or mark anything read. Suppression of represented covered members survives restarts and temporary disable/re-enable of disguise. Attempting to open the chat through the app clears the current cover, including when a password/PIN prompt is cancelled or fails; cover tap/dismiss itself still never marks anything read. Genuinely new members can cover again later. Live incoming/ongoing call UI and stories stay outside disguise, while missed-call service notifications are covered generically. Android still owns app attribution, badges, and any already-pinned old conversation/channel labels. The app itself still cannot change an existing notification channel's importance once that id exists, so cover notifications keep two ready-made channel tiers (silent and alert) and select between them per upstream silent signal; you can still tune either tier further in Android notification settings.
...
Two things it deliberately doesn't do. It silences a chat's own notification, not the grouped summary Android shows above your chat notifications — so that summary can still put a one-line preview of a switched-off chat on the watch. Android decides for itself when to add that summary: usually once two or more chats are unread, but on older Android even a single unread chat gets one, so a lone switched-off chat isn't guaranteed to be fully hidden there. This is the same split Telegram already applies to secret chats (the chat's own notification is kept off the watch, the shared summary is not); it's long-standing behaviour left as-is, not something new. And it covers messages only: an incoming call from a switched-off chat still rings the watch.
...
The schedule picker has a *Send on event* row to set a trigger (e.g., voice, video, text) to send the message early. In that trigger sheet, the round-video row now reads `Video message` and the text row now reads `Any text message` so the type trigger is clearly independent from the optional pattern fields below. The sheet keeps two collapsible groups (`By message type`, `Or by text`) with summary text while collapsed: with an empty setup it opens with only the type group expanded; with any saved condition it opens both groups collapsed. In this sheet only, rows use rounded section cards on the gray background: each trigger group starts with a full-width disclosure row in its own card, followed by that group's controls, and delay stays in its own card below. The pattern help caption sits below the text card and hides with the text group. Collapsing never clears staged choices, and if a hidden text group contains an invalid regex when you tap Done, it expands first and takes you to the exact row error.

Text matching supports up to five patterns on one trigger, evaluated as OR (any one match is enough). The editor starts with one field; *Add pattern* reveals more only when you ask, up to the cap, and each row can be removed in place. The trigger has one shared mode switch for all rows: glob by default (`*` matches anything, `?` one character) or one trigger-wide *Regular expression* mode. Invalid regex rows are flagged inline by row, duplicate rows are kept but noted inline and ignored at save, and the saved summary shows the first pattern plus `+N more`. After a successful Done, the full setup (types, ordered patterns, regex mode, and delay) is remembered per account and restored the next time you start a new trigger.

If triggered, it sends immediately; otherwise, it sends at its scheduled fallback time. Several scheduled messages can share one trigger and will send one by one in fallback-time order, each keeping its own delay. To put that shared trigger on a whole group at once, select them, tap *Reschedule*, and turn on *Send on event* there instead of setting each message by hand. If one send cannot continue right now, the trigger stays armed, a heads-up says the run stopped partway, and matching again continues the rest — unless that chat's notifications are disguised (see *Disguise notifications* above), in which case neither that heads-up nor the one for an early send is shown, so a disguised chat leaks no trigger activity either. The trigger row is unavailable while a message is still being sent (the *Send on event* spot shows a note instead); reopen the schedule sheet once it has finished sending and the row is back. A message is meant to carry only one trigger: editing a message that already has one updates that same trigger instead of adding a second. In the rare case it finds a message already claimed by more than one trigger, it leaves those triggers as they are and changes only the schedule time, with a note that the trigger was left unchanged. Triggers survive an app restart while the scheduled send is still confirming; until a trigger is confirmed again after a restart, changing (or turning off) a trigger anywhere in that chat is declined with the same *trigger was left unchanged* note, while the schedule time still changes — it clears on its own once the confirmation comes back.
...
Muting a video in the gallery editor before you send it normally turns it into a looping GIF — small, fixed low quality, with no scrubber or duration. Now the quality button in the editor's bottom bar, which used to grey out to a "GIF" label the moment you muted, stays live. A fresh mute still lands on GIF, so muting and sending straight away is byte-for-byte what it was before; tap that button once and the clip becomes an ordinary silent video instead — the audio is stripped, but it sends at a quality you pick from the usual SD/HD sheet, with a real duration and a scrubber. The tap takes you from GIF to video; to go back to GIF, unmute and mute again. Avatars, the GIF picker and the documents picker are left exactly as they were.
