...
The same sheet has a `Notifications` section with a `Disguise notifications` switch, off by default. Turn it on and this chat stops showing its name, sender or message text in your notifications; in their place you get a plain generic line (a system-service-style title with a real unread count) so a glance at the lock screen or shade doesn't give the chat away. A `Cover` row appears while it's on, where you pick which generic persona to show from a fixed list (Background service, Build status, Device maintenance, System update, Storage cleanup, Battery optimizer, Sync service, Security check). The persona is assigned once and stays put until you change it, and several chats can share one. Covers are silent and low-priority by default, but the Android channel settings they create are yours to adjust and always win. Turning the switch back off returns the chat to normal notifications and keeps your persona choice for next time.

This is an in-progress guarantee: a covered chat's name, sender and text stay out of the notification, but app attribution, badge counts, calls and stories are still shown, and channels an earlier real notification already created keep their old names in Android Settings.
