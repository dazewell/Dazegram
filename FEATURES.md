...
Two more dynamic Material You themes, Extera Light and Extera Dark, land at the end of the theme strip in Chat Settings, right after Monet AMOLED (Android 12+ only, same as the rest of that row). They reproduce exteraGram's take on the same wallpaper-driven palette instead of ours: everything stays in one neutral hue family rather than shifting to our lightened, desaturated second neutral, and the outgoing bubble is saturated rather than pale. Flat means one hue family, not one tone — pages, section cards, dialogs, the chat canvas, its composer and pinned-message panels, and incoming bubbles each still sit a tone apart from whatever's behind them, so Settings and Profile keep showing white cards on a gray page instead of one slab, and bubbles and panels stay readable against the canvas instead of blending into it. Picking one behaves exactly like the existing Monet themes — colours re-derive when the wallpaper changes, no restart needed, and neither is preselected. Per-peer avatar colours in the chat list stay ours either way: exteraGram flattens every avatar to one colour, which would undo the harmonized colours that make chats easy to tell apart at a glance. The single fixed avatars — Saved Messages and Archive, where there's nothing to tell apart — follow exteraGram's accent like the rest of the palette.
