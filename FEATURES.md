...
### Chat background patterns on Monet themes <!-- #monet-chat-pattern -->

The seven Material You themes that read their palette from your phone — Monet Light, Dark and AMOLED, Extera Light and Dark, Solid Light and Dark — can now carry a chat background pattern, the same dotted and geometric masks the standard themes already offer under Change Chat Background. Pick one the usual way (Settings, Chat Settings, Change Chat Background) and it applies to all seven at once, because they share one pattern rather than storing it per variant. That's what fixes the old problem where a pattern set on one of them vanished the moment the day/night switch flipped to its sibling: there is now a single stored pattern, and the wallpaper screen carries a short footer saying so. The colour under the pattern always comes from the live palette of whichever variant is active, so it re-tints when your Monet colours change instead of freezing the colour it had when you picked it. Clearing the pattern from any one of the seven clears it for all of them, which is intended. On a near-black background like Monet AMOLED the pattern is lifted so it stays visible against true black rather than washing out. Standard (non-Monet) themes are untouched, and adjusting the pattern's contrast works as before. Any pattern you set on these themes before this change needs to be picked once more, since the old per-theme entry can't be carried over cleanly.
