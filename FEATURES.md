...
The Chat Privacy sheet now uses the same rounded section-card surface: one card for `Hide last message` / `Placeholder text` / `Require password`, then one for `Disguise notifications` / `Cover` / `Preview notification`, with bare section headers and disclosure rows outside cards. To keep the sheet short by default without hiding active controls, the old always-open limitations footer is now a collapsed `How covers work` disclosure at the bottom. Expanding it shows the same limitations text in place. On short/landscape screens the sheet content scrolls so every control and the full expanded text stay reachable, and sheet bulletins use Telegram's stock bottom placement.
