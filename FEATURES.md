...
The `<application>` icon and round icon in the app manifest — Android's fixed, non-selectable app identity, used for things like the app switcher and system permission dialogs — differs by package variant. On DazegramX (Unofficial), it's the Default icon art — the same art as the *Default* option in the Chat Settings > App Icon picker. On Dazegram (Official), it keeps its existing Blue icon, unchanged. This is separate from the launcher icon you pick under Chat Settings > App Icon: that picker still works exactly as before, on both variants, and changes what shows on your home screen through its own activity aliases. Choosing a different launcher icon there does not change this fixed `<application>` icon. On the tested ColorOS device, running DazegramX, this fixed icon is confirmed to be what notifications show, instead of the chosen launcher icon or the small notification icon set separately in Nagram Settings; other ColorOS versions or OEM skins may behave differently.
