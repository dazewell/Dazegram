...
Ghost Mode hides read receipts, typing and online status, but on its own it never holds a send back. The first time you type into an empty message box in a chat during a Ghost session, a heads-up points you at Hold Messages; that heads-up starts the chat's quiet period until Ghost Mode is turned off and on again. With Hold Messages enabled, held text, contacts, and static locations use the Scheduled-list caption as feedback instead, while anything that still reaches the network warns before the quiet period starts, including forwards, photos, files, stickers, voice messages, live locations, paid or disappearing messages, and held messages sent by hand.
