...
The schedule sheet's *Default delay* slider always opens the picker the same distance from now — ten minutes by default. Under it there's now a *Remember* switch. Flip it on and the picker stops using the slider: it opens on however far ahead you last scheduled something. Set a message for two hours out and the next sheet opens two hours out too, and the offset shows next to the switch so you know what's coming before the wheels even settle. It's the offset that's kept, not the clock time, so it moves with you through the day. Dragging the slider is a fresh choice of delay, so it turns Remember back off and drops what was saved; turning the switch off drops it as well. Editing an existing scheduled message is untouched, since that sheet opens on the time already set.
