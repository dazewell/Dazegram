...
The separate glass text pill keeps its renderer 16dp from both logical edges and holds Send or mic at its trailing end. A shared glass action capsule sits below it: emoji stays at one edge, attachments holds the other with expand and schedule beside it, and the controls between them scroll only when needed. The capsule now spans the full width of its row, so wherever the text pill runs the full row above it their left and right edges line up flush, whatever the button count or the toolbar size — a sparse row leaves blank glass between the scrolling group and the trailing buttons instead of shrinking to a small centred pill. Quote, Spoiler, Select All, and Clear stay in the toolbar. Quote, Spoiler, and Clear enable when text is selected; Select All enables whenever the field has text and selects the whole draft. Monospace is gone from the row (still there in the long-press selection menu). Toggle any of them off in settings.
