...
The send column no longer rides inside the input card. The glass card now stops short of the send / mic button, so that button (and the slow-mode countdown, the stickers arrow, the cancel-inline-bot cross, the done button while editing, the rich-editor button that appears above the send button once a draft passes two lines) floats beside the card as its own circle instead of sitting on top of it. Buttons that used to paint nothing of their own get the same glass circle the rest of the floating controls wear, so every control on that side reads the same way.
