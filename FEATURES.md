...
The same sheet has a `Notifications` section with a `Disguise notifications` switch, off by default. While it is on, that chat uses a generic cover persona instead of real chat/sender/message content. You can pick the persona (`Cover`) and post a safe `Preview notification` from the sheet without changing unread state. A disguised chat is silent by default (no sound, vibration, or watch alert), because that's what keeps the cover discreet; a per-chat `Alert normally` switch, also off by default, opts a specific chat back into sound, vibration, and your paired watch firing for it - still under the cover identity, never the real chat/sender/message. Turning it off doesn't affect any other chat, even one sharing the same persona, and the choice sticks across turning `Disguise notifications` off and back on for that chat.

Covered taps/dismissals only clear that represented cover and never open the chat or mark anything read. Suppression of represented covered members survives restarts and temporary disable/re-enable of disguise. Attempting to open the chat through the app clears the current cover, including when a password/PIN prompt is cancelled or fails; cover tap/dismiss itself still never marks anything read. Genuinely new members can cover again later. Live incoming/ongoing call UI and stories stay outside disguise, while missed-call service notifications are covered generically. Android still owns app attribution, badges, and any already-pinned old conversation/channel labels. The app itself can never change an existing notification channel's importance once created, in either direction, so `Alert normally` above works by switching a chat between two ready-made channels rather than adjusting one in place - you can still go into Android's own notification settings for either channel and adjust it further, same as any other app notification.
