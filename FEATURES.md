...
Select several messages, use *Forward* with *Hide sender's name* on, pick a chat, then long-press *Send* and choose *Schedule*. When two or more items are going out, the schedule sheet gains an interval row: set a base time and a gap, and each item is scheduled on its own time — base, base + gap, base + two gaps, and so on — instead of every item landing on the same timestamp. That single shared timestamp was why a batch scheduled this way used to move around as a block when you tried to reschedule one of them; giving each its own time is what fixes it, so they now behave as ordinary, independent scheduled messages you can edit one at a time.
