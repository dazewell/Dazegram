...
To keep the sheet short by default without hiding active controls, the old always-open limitations footer is now a collapsed `How covers work` disclosure at the bottom. Expanding it shows the same limitations text in place. On short/landscape screens the sheet content scrolls, so every control and the full expanded text stay reachable.
...
The schedule picker has a *Send on event* row to set a trigger (e.g., voice, video, text) to send the message early. In that trigger sheet, the round-video row now reads `Video message` and the text row now reads `Any text message` so the type trigger is clearly independent from the optional pattern fields below. The sheet now uses two collapsible groups (`By message type`, `Or by text`) with summary text while collapsed: with an empty setup it opens with only the type group expanded; with any saved condition it opens both groups collapsed. Collapsing never clears staged choices, and if a hidden text group contains an invalid regex when you tap Done, it expands first and takes you to the exact row error.
