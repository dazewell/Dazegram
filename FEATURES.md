...
The 7-hue sender identity palette (Red/Orange/Violet/Green/Cyan/Blue/Pink) used to only get a light hue nudge toward the Monet accent — its chroma and tone stayed at their original hardcoded values, so a hue like Blue still read as a fully-saturated designer blue sitting on top of a muted red/brown wallpaper palette, most visibly in the incoming reply/quote bar. It now pulls chroma and tone toward the accent's own HCT values too, on top of the existing hue rotation, so all seven read as part of whatever palette the wallpaper derived instead of one hue looking foreign against it. This is the same identity palette everywhere it already showed up — sender names, reply/quote/code/link/contact bars, folder tags, and the peer color pickers — so it all shifts together rather than just the reply bar. The seven hues stay separated from each other and from the outgoing reply bar; this only changes how well each one fits the palette, not which hue is which. A chroma floor keeps a near-grey wallpaper accent from washing all seven toward the same flat grey. Semantic borrowers that reuse one of these hues for meaning rather than identity (the open-now green in call info, the revenue green in channel monetization, Instant View's danger/success colors) still read recognizably red/green — the pull only moves chroma and tone, never hue. Applies to every Monet-family theme (Monet Light/Dark/AMOLED, Extera Light/Dark, Solid Light/Dark); classic and imported themes are untouched, since this only changes values `MonetHelper` derives at runtime rather than anything baked into a `.attheme` file. The outgoing reply bar and the avatar background colours are unchanged.
