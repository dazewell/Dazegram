...
The toolbar row can be scaled from 75% to 125% in 5% steps. A second slider sets icon spacing, packing buttons closer without shrinking them. Its tightest step puts the buttons' press circles edge to edge; steps below that grey out so icons can't overlap. The Material Design 3 composer keeps 48dp targets, so its spacing only tightens above 100%.
