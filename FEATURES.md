...
Select several messages, then open the forward chat-picker — either with the selection bar's bottom-left *NoQuote* button (its default action) or with stock *Forward* and *Hide sender's name* turned on in the preview — pick a chat, then long-press *Send* and choose *Schedule*. When two or more items are going out, the schedule sheet gains an interval row: set a base time and a gap, and each item is scheduled on its own time — base, base + gap, base + two gaps, and so on — instead of every item landing on the same timestamp. That single shared timestamp was why a batch scheduled this way used to move around as a block when you tried to reschedule one of them; giving each its own time is what fixes it, so they now behave as ordinary, independent scheduled messages you can edit one at a time. The interval row only appears when the forward drops the sender's name — either door does that; a plain *Forward* that keeps the "Forwarded from" header schedules the old way, on one shared time.

Each item becomes its own scheduled send, so a photo/video/document album counts as one slot on one time, not one per file — the interval row's count is slots, while the preview line still counts messages. Because this re-sends rather than forwards, media has to be in the app's cache and is re-uploaded, so you pay for the upload again, and anything Repost as Copy can't re-send (polls, locations, contacts) is refused up front with a count of how many, before anything is sent. The check happens once for the whole selection, so it's all-or-nothing: nothing goes out unless every slot passed. The cache check is a snapshot, though — it reserves nothing, so if a file is evicted or its upload fails after that point, that slot may simply not go out, or it may land as a failed message you can retry; either way the delete-originals offer below stays off for the run. While the copies are still uploading they aren't on the server's schedule yet, so for that short window they won't show in the scheduled list; they appear once each upload confirms. "Absent" here means absent from the server's scheduled set — a slot the server rejects can still leave a failed placeholder on your side to retry, same as any failed send.

Sending to a single chat still offers to delete the originals once the whole batch confirms, exactly as a plain Repost as Copy does; spreading to more than one chat never offers it. Opening a photo full-screen and forwarding from there keeps Telegram's ordinary forward, so a batch scheduled that way still shares one time — use the selection bar's *Forward* for the spread.
