...
The inherited updater that polls upstream's release channel and nags you to install an upstream APK over your own build is now off by default. Update Channel in Settings still has its Off / Release / Beta picker and the Check for Updates row, so you can opt back in whenever you want, but a fresh install and any install that never touched the setting both read Off now. While it's off, nothing leaves the device: the automatic check on resume stays silent, and every explicit tap — Check for Updates, the debug menu's check, the tg://update link, the in-chat "needs a newer app" button, the stories-viewer one — shows a short bulletin saying checks are off instead of contacting the channel. The persistent "Update NagramX" drawer banner can't appear while off, a release marked as unskippable can no longer force the full-screen blocking view, and any stale pending update already saved to disk is dropped on the next launch so it can't wall you out. Selecting Release or Beta from the picker runs its check as before.
