...
The `<application>` icon and round icon in the app manifest — Android's fixed, non-selectable app identity, used for things like the app switcher and system permission dialogs — is Dazegram's Default icon art. This is separate from the launcher icon you pick under Chat Settings > App Icon: that picker still works exactly as before and changes what shows on your home screen through its own activity aliases. Choosing a different launcher icon there does not change this fixed `<application>` icon. Some OEM skins, including tested ColorOS builds, are known to substitute a fixed application icon for the notification icon rather than the chosen launcher icon or the small notification icon set separately in Nagram Settings; on those skins this fixed icon is what notifications may show instead of your selected launcher icon.
