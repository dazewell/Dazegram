...
**This is a personal fork** by [@dazewell](https://github.com/dazewell) with a few extras on top of [NagramX](https://github.com/risin42/NagramX):
 ...
 - **Require password:** when an app passcode is set, flag a chat from its in-chat ⋯ menu to ask for the passcode (or fingerprint) before it opens. It doesn't matter how you get there: a tap, a notification, a shortcut, a search result, or a long-press peek all hit the same lock. Turning this on also hides the chat's last message in the list. Once you unlock the chat it stays open until you put the app in the background, then it asks again.
