...
- Branches are `<YYYY-MM-DD>_<slug>`; the date prefix is mandatory and
  `process-rules.yml` accepts `-` as an equally valid separator. With the hooks
  enabled, `.githooks/post-commit` dates an undated local branch at its first
  commit and `.githooks/pre-push` blocks pushing one.
