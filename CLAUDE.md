...
The skill, this file, and the persistent memory (`MEMORY.md` + its per-feature maps) are the source of truth for how we work. When a rule, convention, or workflow changes, update all three in the same session the change is decided, not later. If a correction reveals an existing instruction is wrong or stale, fix it rather than leaving it.
