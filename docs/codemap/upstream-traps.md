...
One live behaviour to expect around all this: GitHub computes `mergeable`/`mergeStateStatus` **asynchronously**. After ruleset `22861936` was created, open PRs read `mergeable: UNKNOWN` for minutes before settling to `CLEAN`, and a PR that had already MERGED read `UNKNOWN` indefinitely (2026-09-10). So `mergeable: MERGEABLE` is never a freshness guarantee — it answers "does this textually merge", not "is this green" — and the instant any merge moves `dev`, every other open PR's `mergeable` drops back to `UNKNOWN` until a background job recomputes it. Gate on `mergeStateStatus == CLEAN` plus a head-pinned green `ci.yml` run, re-read each time, never on a cached or just-observed `MERGEABLE`.
