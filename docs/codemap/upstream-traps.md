...
- **Why #334's body came out empty is unresolved — two candidate causes, and the evidence does not decide between them.** Either (a) `squash_merge_commit_message` was **not yet** `COMMIT_MESSAGES` when #334 merged, or (b) the squash message was **hand-edited or cleared in the merge UI** for that one PR. Merge times: #334 `02:46:55Z`, #335 `02:54:25Z`, #338 `03:26:17Z` — #334 landed its empty body **eight minutes before** #335 landed with its tags intact, which fits either a setting corrected in that window or a manual edit on #334 alone. **Do not pick one.** A future reader who "fixes" this by mandating the tag in the PR *body* would be chasing the wrong cause: subject-only tags already survive under `COMMIT_MESSAGES` today, per the `* <subject>` behaviour above.
