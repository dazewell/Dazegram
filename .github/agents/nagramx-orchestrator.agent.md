...
# the hard line, mechanically. one pattern for both, so they cannot drift apart
$ai = 'Co-Authored-By|Generated with|Copilot|Claude|Anthropic|OpenAI|AI-generated|generated by|assistant'
git log origin/dev..origin/$branch --format='%an|%ae|%s%n%b' | Select-String -Pattern $ai
git diff origin/dev...origin/$branch -- '*.java' '*.kt' '*.xml' |
  Select-String -Pattern "^\+.*($ai)"
