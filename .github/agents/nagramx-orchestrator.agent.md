...
gh run view <databaseId> --repo $repo --json jobs --jq '.jobs[]|{name,conclusion}'
...
# the hard line, mechanically
git log origin/dev..origin/$branch --format='%an|%ae|%s%n%b' |
  Select-String -Pattern 'Co-Authored-By|Generated with|Copilot|Claude|Anthropic|OpenAI|assistant'
git diff origin/dev...origin/$branch -- '*.java' '*.kt' '*.xml' |
  Select-String -Pattern '^\+.*(Copilot|Claude|Anthropic|AI-generated|generated by)'
...
gh api graphql -f query="{repository(owner:\"dazewell\",name:\"NagramX\"){pullRequest(number:$pr){reviewThreads(first:100){nodes{isResolved path line}}}}}"
```
