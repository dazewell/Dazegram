...
9. **Build it, hand it to dazewell to test on-device.** Don't open a PR
   ...
   To produce a testable build, push the branch and trigger the staging
   workflow against it: `gh workflow run staging.yml --ref <branch>`. dazewell
   tests from the **uploaded** artifact, so let the upload job run — do **not**
   pass `-f upload=y` and don't put `[skip upload]` in the commit. The build
   uploads the Unofficial + Official APKs through the normal channel; that's
   what dazewell installs from.
